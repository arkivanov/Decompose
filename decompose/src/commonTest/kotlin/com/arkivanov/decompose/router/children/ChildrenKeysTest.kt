...
class ChildrenKeysTest : ChildrenTestBase() {
    ...
    @Test
    fun WHEN_created_children_with_same_configurations_added_to_end_THEN_new_unique_key_generated() {
        val children by context.children(initialState = stateOf(1 by DESTROYED))
        val keys = children.keys

        navigate { it + listOf(2 by CREATED, 2 by CREATED) }

        assertEquals(keys, children.keys.dropLast(2))
        children.assertKeysUnique()
    }
    ...
    @Test
    fun WHEN_created_children_with_same_configurations_added_to_front_THEN_new_unique_key_generated() {
        val children by context.children(initialState = stateOf(1 by DESTROYED))
        val keys = children.keys

        navigate { listOf(2 by CREATED, 2 by CREATED) + it }

        assertEquals(keys, children.keys.drop(2))
        children.assertKeysUnique()
    }
    ...
    @Test
    fun GIVEN_multiple_children_with_same_configurations_WHEN_configuration_change_THEN_keys_not_changed() {
        var ctx = TestComponentContext()
        var children = ctx.children(initialState = stateOf(1 by DESTROYED, 1 by CREATED, 1 by STARTED, 1 by RESUMED))
        val keys = children.value.keys
    ...
    @Test
    fun GIVEN_multiple_children_with_same_configurations_WHEN_recreated_THEN_keys_not_changed() {
        var ctx = TestComponentContext()
        var children = ctx.children(initialState = stateOf(1 by DESTROYED, 1 by CREATED, 1 by STARTED, 1 by RESUMED))
        val keys = children.value.keys
