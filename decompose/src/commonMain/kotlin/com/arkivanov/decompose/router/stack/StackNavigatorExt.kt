 ...
 * in the stack. However, duplicate configurations can be enabled by setting
 * [com.arkivanov.decompose.DecomposeParameters.duplicateConfigurationsEnabled] flag to `true`.
 *
...
inline fun <C : Any> StackNavigator<C>.pushToFront(
    ...
    navigate(
        transformer = { stack ->
            ...
            val existingIndex = mutableStack.lastIndexOf(configuration)
            if (existingIndex >= 0) {
                mutableStack.removeAt(existingIndex)
            }

            mutableStack.add(configuration)
