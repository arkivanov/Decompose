...
fun <Ctx : GenericComponentContext<Ctx>, C : Any, T : Any> Ctx.childItems(
    ...
    return object : LazyChildItems<C, T>(), ItemsNavigator<C> by navigator {
        override val value: ChildItems<C, T> get() = navigator.state.value

        override fun subscribe(observer: (ChildItems<C, T>) -> Unit): Cancellation =
            navigator.state.subscribe(observer)
    }
}
