...
internal class ChildrenNavigator<out C : Any, out T : Any, N : NavState<C>>(
    ...
    private fun restore(navState: N, savedStates: List<SerializableContainer?>) {
        val retainedItems =
            if (navState.children.mapped { it.configuration } == items.mapped { it.configuration }) {
                items.mapIndexed(::Pair).toMap(HashMap())
            } else {
                null
            }
    ...
    private fun switch(newStates: List<ChildNavState<C>>) {
        ...
        if (!DecomposeParameters.duplicateConfigurationsEnabled) {
    ...
    private fun prepareNewItems(
        newStates: Map<ItemKey<C>, ChildNavState<C>>,
        oldItems: Map<ItemKey<C>, ChildItem<C, T>>,
