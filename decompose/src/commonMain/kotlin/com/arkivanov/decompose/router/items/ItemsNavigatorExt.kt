 ...
 * Replaces the items with the provided list.
 *
 * @param items see [Items.items].
 */
@ExperimentalDecomposeApi
fun <C : Any> ItemsNavigator<C>.setItems(
    items: List<C>,
    onComplete: (newItems: Items<C>, oldItems: Items<C>) -> Unit = { _, _ -> },
) {
    setItems(items = { items }, onComplete = onComplete)
}
 ...
 * See [ActiveLifecycleState].
 *
 * @param activeItems see [Items.activeItems].
 * @param onComplete called when the navigation is finished (either synchronously or asynchronously).
 */
@ExperimentalDecomposeApi
fun <C : Any> ItemsNavigator<C>.setActiveItems(
    activeItems: Map<C, ActiveLifecycleState>,
    onComplete: (newItems: Items<C>, oldItems: Items<C>) -> Unit = { _, _ -> },
) {
    setActiveItems(activeItems = { activeItems }, onComplete = onComplete)
}
